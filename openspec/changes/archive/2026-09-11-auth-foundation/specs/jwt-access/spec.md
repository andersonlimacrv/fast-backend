...
The system SHALL include `active_org_id` in the access token as context (membership enforcement belongs to the tenancy phase, not this change).

#### Scenario: Switch organization mints a new token
- **WHEN** an authenticated user calls `POST /auth/switch-organization` for an organization
