/* Vendored from @animate-ui/components-radix-dropdown-menu (user paste, 2026-09-16).
 * Local adaptations: 'use client' removed (Vite); CheckIcon/ChevronRightIcon/
 * CircleIcon (lucide) → Check/ChevronRight/CircleDot (@/lib/icons, golden rule).
 * asChild/Slot usage is upstream-owned (exception documented in design.md). */
...
function DropdownMenuContent({
  ...
  return (
    <DropdownMenuContentPrimitive
      ...
      className={cn(
        'bg-popover text-popover-foreground relative z-50 max-h-(--radix-dropdown-menu-content-available-height) min-w-[8rem] origin-(--radix-dropdown-menu-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-md border p-1 shadow-md outline-none',
        className,
      )}
      {...props}
    >
      ...
      >
        <span className="pointer-events-none absolute left-2 flex size-3.5 items-center justify-center">
          <DropdownMenuItemIndicatorPrimitive layoutId="dropdown-menu-item-indicator-radio">
            <CircleDot className="size-2 fill-current" aria-hidden="true" />
          </DropdownMenuItemIndicatorPrimitive>
        </span>
        {children}
      </DropdownMenuRadioItemPrimitive>
...
function DropdownMenuSubTrigger({
  ...
  return (
    <DropdownMenuHighlightItemPrimitive disabled={disabled}>
      ...
      >
        {children}
        <ChevronRight data-slot="chevron" className="ml-auto size-4" aria-hidden="true" />
      </DropdownMenuSubTriggerPrimitive>
    </DropdownMenuHighlightItemPrimitive>
  );
}
...
function DropdownMenuSubContent({
  ...
  return (
    <DropdownMenuSubContentPrimitive
      className={cn(
        'bg-popover text-popover-foreground relative z-50 min-w-[8rem] origin-(--radix-dropdown-menu-content-transform-origin) overflow-hidden rounded-md border p-1 shadow-lg outline-none',
        className,
      )}
      {...props}
    />
