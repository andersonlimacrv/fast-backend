/* Vendored from @animate-ui/primitives-radix-dropdown-menu (registry, 2026-09-16).
 * Local adaptations: 'use client' removed (Vite); Content/SubContent render a
 * plain radix positioned wrapper with the motion.div INSIDE — radix injects
 * position/translate via asChild Slot and motion swallows it (menus landed at
 * 0,0 unpositioned), while Sheet/tooltip carry explicit styles and are unaffected.
 * Wrapper carries style zIndex 50: floating-ui positions it with transform
 * (which traps inner z-index in a z-auto context), so without this the menu
 * paints/taps UNDER modal siblings (Sheet z-50) instead of above them.
 * NOTE (popper mechanics): @radix-ui/react-popper copies the CONTENT's
 * computed z-index onto its positioned wrapper, so the Content element itself
 * must be positioned (relative) with zIndex 50 — styling only the inner
 * motion.div is not enough (it stays trapped in the wrapper's context).
 * Re-fetch from https://animate-ui.com/r/primitives-radix-dropdown-menu.json to update. */
