/* Vendored from @animate-ui/primitives-effects-highlight (registry, 2026-09-16).
...
function HighlightItem<T extends React.ElementType>({
  ...
  const dataAttributes = {
    ...
    // NOTE (local adaptation): upstream also sets 'aria-selected' here, but it
    // is invalid ARIA on generic elements (axe aria-allowed-attr). Selection is
    // conveyed by data-active styles + NavLink aria-current instead.
    'data-disabled': isDisabled,
    'data-value': childValue,
    'data-highlight': true,
  };

  // Local adaptation: compose outer handlers (e.g. TooltipTrigger's, passed
  // through Slot) with the highlight's own — upstream overwrote props handlers
  // here, silently dropping Tooltip open/close in SidebarMenuButton.
  type Handler = ((e: React.SyntheticEvent) => void) | undefined;
  const outer = props as Record<string, Handler | unknown>;
  const outerEnter = outer.onMouseEnter as Handler;
  const outerLeave = outer.onMouseLeave as Handler;
  const outerClick = outer.onClick as Handler;
