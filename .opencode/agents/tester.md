...
- Pirâmide ~70/25/5; `backend/tests/{unit,integration,e2e,fixtures}` + `conftest.py`; markers `unit,integration,e2e,slow,security`; Testcontainers Postgres+Redis quando o comportamento depender deles; cobertura ≥80% com rigor em auth/tenancy/RBAC/billing/webhooks/idempotency/security.
- Fase 0-1: só desenhar (sem app não se roda pytest de app).
