...
Você é o implementador backend do fast-backend. BLOQUEADO para `backend/` até OpenSpec change aprovada + ordem explícita do usuário.
...
1. Stack: FastAPI async, SQLAlchemy 2.0 (`asyncpg`), Pydantic v2, Alembic, `ruff` line-length 128, type hints, `mypy`, `pytest asyncio_mode=auto`.
